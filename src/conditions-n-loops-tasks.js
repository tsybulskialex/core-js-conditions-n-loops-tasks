...
function getSpiralMatrix(size) {
  ...
  while (cur <= size * size) {
    ...
    endCol -= 1;
    for (let k = endCol - 1; k >= startCol; k -= 1) {
      arr[endRow - 1][k] = cur;
      cur += 1;
    }
    endRow -= 1;
    for (let h = endRow - 1; h >= startRow; h -= 1) {
      arr[h][startCol] = cur;
      cur += 1;
    }
    startCol += 1;
...
function rotateMatrix(matrix) {
  ...
  for (let j = 0; j < copyArr.length; j += 1) {
    for (let k = 0; k < copyArr.length; k += 1) {
      newArr[j][k] = copyArr[copyArr.length - 1 - k][j];
    }
  }
...
function sortByAsc(arr) {
  ...
  for (let i = 0; i < newArr.length; i += 1) {
    let cur = newArr.length - 1 - i;
    for (let j = 0; j < newArr.length - i; j += 1) {
      if (newArr[j] > newArr[cur]) {
        cur = j;
      }
    }
    [newArr[newArr.length - 1 - i], newArr[cur]] = [
      newArr[cur],
...
function shuffleChar(str, iterations) {
  ...
  while (total !== 0) {
    ...
    total -= 1;
    if (str === newStr) {
      const numRepeat = iterations - total;
      total %= numRepeat;
    }
  }
  return newStr;
